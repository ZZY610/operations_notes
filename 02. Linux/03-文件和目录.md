...
FHS（Filesystem Hierarchy Standard）是一个用于规范Linux文件系统目录结构的标准。其目的是统一不同Linux发行版、开发者和用户对文件系统目录的理解和使用方式，让用户能够知道已安装软件通常放置在哪个目录下。

FHS的重点在于规范每个特定目录应该存放哪种类型的数据。
...
在类 Unix 系统中并不存在 C/D/E/F 盘符,一切的文件都是从 “ 根 (/)” 目录开始的。按照文件系统目录标准 FHS 采用树形结构来存放文件，并定义了每个区域的用途。
...
| :---------- | -- | :------------------------------- |
| / | 根目录 | 一切从这里开始 |
| /bin        | Binary 二进制文件 | 系统启动运行所必须的二进制文件 |
| /boot       | Boot 引导 | 开机所需文件 —— 内核,开机菜单及所需配置文件等 |
| /dev        | Device 设备 | 存储外部硬件设备，以文件的形式 |
| /etc        | Etcetera 系统配置 | 系统层面配置文件，也包含shell脚本 |
| /home       | Home 主目录 | 用户主目录 |
| /lib        | Library 库文件 | 存放必要的运行库，包括开机时用到的函数库、/bin 与/sbin 下面命令要调用的函数、用户程序所需库等 |
| /sbin       | System Binary 系统二进制文件 | 系统管理和维护相关的可执行文件。 |
| /media      | Media 媒体 | 挂载可移动媒体设备的目录，如光盘、硬盘等。 |
| /opt        | Optional 可选 | 放置第三方的软件 |
| /root       | Root 根 | 系统管理员的主文件夹 |
| /var        | Variable 可变 | 主要存放经常变化的文件，如日志。 |
| /tmp        | Temporary 临时文件 | 任何人均可使用的 “ 共享 ” 临时目录 |
| /proc       | Process 进程 | 虚拟文件系统，内容动态生成，可以获取例如系统内核、进程、外部设备及网络状态等相关信息 |
| /usr | Unix System Resources 系统资源 | 存放用户级别的系统资源，包括用户安装的软件、库文件、文档等。 |
| /usr/bin | | linux发行版安装的可执行程序，通常包含成千上万个程序 |
| /usr/local  |  | 用户自行安装的软件 |
| /usr/local/bin | | 用户本地安装的软件，不是通过包管理工具。 |
| /usr/sbin   | | 非系统开机时需要的软件/命令/脚本，存储系统管理任务所需的系统命令和工具，通常需要管理员权限运行。 |
| /usr/share  |  | 帮助与说明文件,也可放置共享文件。 |
| /lost+found |  | 当文件系统发生错误时，将一些丢失的文件片段存放在这里 |
| /srv        |  | 一些网络服务的数据目录 |

### 路径
路径指的是如何找到某个文件，分为 **绝对路径** 与 **相对路径** 。

* 绝对路径 (absolute): 由根目录 (/) 开始写起的目录或文件名。
* 相对路径 (relative): 相对于当前路径的写法。`·`代表当前目录，`..`代表父目录。
...
SGID
* 范围 : 目录或者拥有可执行权限的文件
* 作用 : 对文件来说，程序以文件属主的权限运行；对目录来说，其中创建的新文件继承目录所属组。
...
#### chmod
chomod options mode file

有两种更改读写执行权限的方法：数字设置和符号设置。
...
Linux 操作系统的文件权限 (rwx) 与文件属性 ( 拥有者、群组、 时间参数等 ) , 文件系统会将这两部分的数据分别存放在不同的区块 , 权限不属性放置到 inode 中 , 实际数据则放置到 data block 区块中。

另外 , 还有一个超级区块 (superblock) 会记录整个 文件系统的整体信息 , 包括 inode 和 block 的总量、使用量、剩余量等。
...
* 软链接也称为符号链接( symbolic link )即 “ 仅仅包含它索要链接文件的路径名 ” 因此能做目录链接也可以跨越文件系统,但原始文件被删除后链接文件也将失效,如同 WinodwTM 中的 “ 快捷方式 ” 。
