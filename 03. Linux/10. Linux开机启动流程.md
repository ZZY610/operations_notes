...
::: tip 概览
开机 → BIOS 启动 → BIOS 加载硬盘  MBR 中的 Grub → Grub 加载 Linux 内核和 initrd 镜像文件到内存中 → Grub 将控制权交给 Linux  内核，Linux 内核开始执行，进行初始化 → Linux 内核将控制权交给 init 程序 → init  程序负责启动后面的所有后台服务软件和前台界面软件。
:::
