...
### 从源码包安装软件的过程
简而言之，就是要把源代码被翻译成机器可执行的二进制文件，并安装到系统中。
1. **下载源码包**：
   - 源码包通常以 `.tar.gz`、`.tar.bz2` 或 `.zip` 等格式压缩分发。这些包包含了软件的源代码以及构建和安装软件所需的其他文件。

2. **解压缩源码包**：
   - 使用 `tar` 命令（例如 `tar -xzf package.tar.gz`）或其他解压工具将源码包解压到一个目录中。解压后，你会得到源代码文件、配置文件和构建脚本（如 `Makefile`）。

3. **配置构建环境**：
   - 通常，解压后的源码包会包含一个名为 `configure` 的脚本。运行 `./configure` 脚本来配置构建环境。这一步会检查你的系统环境和依赖库，并生成适当的 `Makefile`。

4. **编译源代码**：
   - 使用 `make` 命令调用 `Makefile` 中定义的规则来编译源代码。`make` 会依次调用编译器（如 `gcc`）将源代码（如 `.c` 文件）编译成目标文件（`.o` 文件），然后链接这些目标文件生成可执行的二进制文件。这个过程中包括：
     - **预处理**：编译器将源代码中的宏展开和头文件包含等操作。
     - **编译**：将预处理后的源代码翻译成汇编语言。
     - **汇编**：将汇编语言翻译成机器语言生成目标文件。
     - **链接**：将多个目标文件和库文件链接成最终的可执行文件。

5. **安装软件**：
   - 使用 `make install` 命令，将编译生成的可执行文件、库文件和其他资源文件复制到系统的适当位置（如 `/usr/local/bin`、`/usr/local/lib` 等）。
...
**`make`** 命令主要用于自动化构建过程。它根据名为 `Makefile` 的文件中的指令来编译代码。`Makefile` 中包含了一系列的规则和依赖关系，描述了如何生成程序的各个部分。

- **作用**：
  1. **编译程序**：将源代码（如 .c 文件）编译成可执行文件或库（如 .o 文件、.a 文件）。
  2. **管理依赖关系**：自动识别哪些文件已被修改，只重新编译这些文件，从而节省时间。
  3. **自动化任务**：除了编译，`make` 还可以用于自动化其他任务，如清理构建目录、运行测试等。

- **基本使用**：
  1. **编译**：在项目根目录下运行 `make` 即可自动查找 `Makefile` 并执行其中的规则。
  2. **指定目标**：可以通过 `make target` 指定要执行的目标。例如，`make clean` 通常用于清理构建文件。

#### make install 命令

**`make install`** 通常用于将编译好的程序或库安装到系统中。这一步通常会将文件复制到系统的标准目录，如 `/usr/local/bin`、`/usr/local/lib` 等。

- **作用**：
  1. **安装程序**：将可执行文件、库文件、头文件等复制到系统的标准目录中，使其可供系统中的其他软件使用。
  2. **创建目录**：如果必要，`make install` 会创建安装所需的目录结构。
  3. **设置权限**：确保安装后的文件具有正确的权限和所有权。

- **基本使用**：
  1. **运行**：在项目根目录下运行 `make install`。前提是必须首先运行 `make` 进行编译。
  2. **需要权限**：安装步骤通常需要管理员权限，可能需要通过 `sudo make install` 来执行。
...
- **编译**：运行 `make`，`make` 会根据 `Makefile` 编译 `hellomake`。
- **安装**：如果 `Makefile` 中有如下部分：
...
运行 `make install` 会将 `hellomake` 可执行文件复制到 `/usr/local/bin`。
