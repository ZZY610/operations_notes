...
在分析TCP四次挥手释放连接时，我发现了一个问题。客户端向服务端发送连接释放报文（分组7）后，收到的回复报文（分组8）并不是回复分组7的。分组8的Seqnumber和Acknumber并不与分组7匹配。回复了分组7的报文是连接释放报文分组10。然后客户端回复了一个TCP普通确认报文分组11，连接释放完成。
